...
// ============================================================================
// 🔧 [NEW] 시설관리자 전용: 호실별 가전 맵 컴포넌트 (모든 가전 상태 한눈에 파악)
// ============================================================================
function FMFloorPlan() {
  const [selectedRoom, setSelectedRoom] = useState<any>(null);

  // 간호사 뷰와 동일한 12개 병실 + 모든 가전(미가동 포함) 상세 데이터
  const FM_ROOMS = [
    ...
    // 🚨 202호: 환자 고열, CO2 초과로 인한 전체 가전 풀가동 (Heavy Load)
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-4">
        {FM_ROOMS.map((room, i) => (
          ...
          >
            ...
            {/* 💡 각 방의 가전 3개 상태를 한눈에 볼 수 있는 요약 리스트 */}
                ...
                return (
                  <div key={idx} className="flex items-center justify-between text-xs bg-slate-900/50 px-2 py-1.5 rounded-lg border border-slate-800/50">
                    <div className="flex items-center gap-1.5">
                      <span className={isOn ? (room.isHeavyLoad ? 'text-red-400' : 'text-blue-400') : 'text-slate-600'}>
                        {d.icon}
                      </span>
                      <span className={isOn ? 'text-slate-300 font-medium' : 'text-slate-500'}>
                        {getDeviceLabel(d.type)}
                      </span>
                    </div>
                    <span className={`font-bold ${isOn ? (room.isHeavyLoad ? 'text-red-300' : 'text-white') : 'text-slate-600'}`}>
                      {isOn ? d.mode : 'OFF'}
                    </span>
                  </div>
                );
           ...
           <table className="w-full text-left mt-2 border-collapse">
            ...
            <tbody>
              ...
              <tr className="hover:bg-slate-800/30 transition">
                <td className="py-4 pl-4 font-bold text-lg">302호</td><td className="font-medium">한소희 (80세)</td><td className="text-red-400 font-bold text-lg">38.8°C</td><td className="text-red-400 font-bold text-lg">110 bpm</td><td className="text-right pr-4"><span className="bg-red-900/50 text-red-200 px-3 py-1.5 rounded-lg text-xs font-bold inline-block">고열</span></td>
              </tr>
            </tbody>
          </table>
        </Modal>
    ...
    <div className="space-y-8 animate-in fade-in duration-500">
      <div className="grid grid-cols-4 gap-6">
        ...
        <div onClick={() => setFmModal("FILTER")} className="bg-[#111827] border border-[#A50034]/50 rounded-2xl p-6 shadow-lg flex flex-col justify-center cursor-pointer hover:bg-slate-800 transition group relative overflow-hidden">
          ...
          <p className="text-3xl font-black text-[#A50034] pl-1 relative z-10">2<span className="text-sm font-normal ml-1 text-slate-500">대 필요</span></p>
        </div>
      ...
      {/* 💡 새로운 인터랙티브 병실 가전 맵 (미가동/가동/모드 전체 표기) */}
      <div className="space-y-4">
