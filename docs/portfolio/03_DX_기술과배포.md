...
> LG전자 DX SCHOOL 5기 최종 통합 프로젝트 · 5팀 「5분 대기조」 · 🏆 DX 장려상 · DX/엔지니어링 트랙
...
![시스템 아키텍처](../발표/deck_site/시스템%20아키텍처.png)
...
> **정직성 노트**: 실제 운영·부하검증은 §8·§9의 단일 인스턴스(systemd) 경로로 수행했다. 위 7서비스 Compose 중 `sentinel-protocol`(`Dockerfile.mcp`)·`sentinel-reporter`(`Dockerfile.reporter`)는 아직 전용 Dockerfile이 레포에 없어 **미빌드(PoC 설계 슬롯)** 이며, 실동작 백엔드는 `backend/Dockerfile`(및 단일 uvicorn) 기준이다.
...
> `tests/`에는 PoI·tier·과도기·carry-forward·외부 boost·SmartThings 어댑터·인증 등 **10개 테스트 파일 + smoke 시나리오**(총 pytest 74 케이스)를 보유.

### 배포 — AWS EC2 (최종 운영, 현재 아카이브)

- **최종 배포**: AWS EC2(서울, ap-northeast-2) **4 vCPU / 15 GiB RAM** 인스턴스(`<EC2_INSTANCE_ID>`, 공인 IP `<DEMO_VM_HOST>`)로 이전·운영.
- **systemd 서비스**로 기동: `sentinel-backend.service`(uvicorn :8103) · `sentinel-frontend.service`(next :3100).
- **nginx** 리버스 프록시([`infra/nginx-sentinel.conf`](../../infra/nginx-sentinel.conf))로 백엔드/프론트/Grafana(`/sentinel-grafana` subpath, `GF_SERVER_SERVE_FROM_SUB_PATH=true`)를 서빙.
- 엣지(라파이/노트북)는 **Tailscale 메시 VPN**으로 서버에 연결, 서버가 공인 IP로 대시보드/카메라 중계 → 시청 기기가 Tailnet 밖이어도 동작.
- 대회 종료로 서비스 중지 → **아카이브**됨.

> **개발/부하검증 환경(이전 단계)**: 개발 및 2026-06-08 부하검증은 GCP e2-standard-2(2 vCPU) **공용 dev 박스**(`uis-capstone`, <DEV_BOX_HOST>)에서 수행했다(아래 §9). 이후 운영 배포를 위해 AWS EC2로 이전했다.
...
> **정직성 노트**: [`.github/workflows/deploy.yml`](../../.github/workflows/deploy.yml)은 AWS EC2 SSH 배포 워크플로(workflow_dispatch 수동 트리거 → git pull → 의존성 설치 → systemd 재시작 → 헬스체크)로 구현. VM_HOST·VM_USER·VM_SSH_KEY Secrets 등록 시 즉시 동작(현재 데모 종료로 미가동).
...
| **deploy.yml 배포** | ✅ workflow_dispatch AWS EC2 SSH 배포(수동 트리거) |
